...
| **한 음절 발화가 EMPTY로 버려진다** | **실측 — 결함** | ⚠️ `is_empty_transcript`가 `len <= 1`이라 `BACKCHANNEL_WORDS` 20개 중 **10개(네 넵 아 어 예 오 와 음 응 헐)가 도달 불가**. 게다가 `"뭐?"` `"왜?"` 같은 정상 반말 질문도 무음으로 버려지고 **"마이크 죽었다" 경고 카운터를 올린다**. 아래 사람 검증 항목 참고 |
